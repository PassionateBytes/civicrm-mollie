<table class="crm-info-panel mollie-detail-panel">
  ...
  {if !empty($item) && ($item.dashboard_url || $item.related_links || $item.documentation_url)}
    <tr>
      <td class="label">{ts domain="nl.stichtinggast.mollie"}Links{/ts}</td>
      <td class="mollie-detail-item-links">
        {if $item.dashboard_url}
          <a href="{$item.dashboard_url|escape}" onclick="window.open(this.href); return false;"><i class="crm-i fa-external-link"></i> {ts domain="nl.stichtinggast.mollie"}Open in Mollie{/ts}</a>
        {/if}
